    ...
    if len(args[3]) > 0:
        if args[3][0] == "list":
            ...
            if args[0].gathering == "gathering=no":
                ...
                name = random_mob[1]
                hp = round(stats[difficulty][0] * (1 + (0.1 * random.randint(-3, 3))))
                dmg = round(stats[difficulty][1] * (1 + (0.1 * random.randint(-3, 3))))
                loot = round(stats[difficulty][2] * (1 + (0.1 * random.randint(-3, 3))))
